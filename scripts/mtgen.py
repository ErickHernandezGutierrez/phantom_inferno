...
parser = argparse.ArgumentParser(description='Generate phantom multi-tensor diffusivities.')
...
for sub_id in range(nsubjects):
    ...
    nib.save( nib.Nifti1Image(lambdas, np.identity(4)*vsize), '%s/ground_truth/diffs.nii'%(subject) ) 
    nib.save( nib.Nifti1Image(ad, np.identity(4)*vsize), '%s/ground_truth/ad.nii'%(subject) ) 
    nib.save( nib.Nifti1Image(rd, np.identity(4)*vsize), '%s/ground_truth/rd.nii'%(subject) ) 
    nib.save( nib.Nifti1Image(fa, np.identity(4)*vsize), '%s/ground_truth/fa.nii'%(subject) ) 
    nib.save( nib.Nifti1Image(md, np.identity(4)*vsize), '%s/ground_truth/md.nii'%(subject) ) 
