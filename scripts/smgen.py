...
parser.add_argument('--d_par_ic',  type=float, default=[2e-3], nargs='+', help='IC parallel diffusivities. default: 1.7e-3 mm^2/s')
parser.add_argument('--d_par_ec',  type=float, default=[2e-3], nargs='+', help='EC parallel diffusivity. default: 1.7e-3 mm^2/s')
...
if len(d_par_ic) == 1:
    ...
    d_par_ic = np.repeat( d_par_ic, nsubjects*X*Y*Z ).reshape(nsubjects, X,Y,Z)
...
if len(d_par_ec) == 1:
    ...
    d_par_ec = np.repeat( d_par_ec, nsubjects*X*Y*Z ).reshape(nsubjects, X,Y,Z)
...
if len(d_perp_ec) == 1:
    ...
    d_perp_ec = np.repeat( d_perp_ec, nsubjects*X*Y*Z ).reshape(nsubjects, X,Y,Z)
...
for i in range(nbundles):
    mask[:,:,:, i] = nib.load('%s/bundle-%d__wm-mask.nii' % (phantom,i+1)).get_fdata()
...
for sub_id in range(nsubjects):
    ...
    # save diffusivities
    nib.save( nib.Nifti1Image(diffusivities, np.identity(4)*vsize), '%s/ground_truth/diffs.nii'%(subject) ) 
