...
for sub_id in range(1, nsubjects+1):
    ...
    print('preparing %s for tractometry' % subject)

    print('\tcreating input folder for tractometry')
    if not os.path.exists( '%s/input_tractometry' % study ):
        os.system('mkdir %s/input_tractometry' % study)
    if not os.path.exists( '%s/input_tractometry/%s' % (study,subject) ):
        os.system('mkdir %s/input_tractometry/%s' % (study,subject))
    if not os.path.exists( '%s/input_tractometry/%s/metrics' % (study,subject) ):
        os.system('mkdir %s/input_tractometry/%s/metrics' % (study,subject))
    if not os.path.exists( '%s/input_tractometry/%s/bundles' % (study,subject) ):
